...
Now uses Pydantic models for type safety.
"""
from typing import Any

from utils.schemas import GraphEdge, GraphNode, KnowledgeGraph
...
def deduplicate_graph(graph: dict[str, Any] | KnowledgeGraph) -> KnowledgeGraph:
    ...
    if isinstance(graph, KnowledgeGraph):
        ...
        nodes = [GraphNode(**n) for n in nodes_dict]
        edges = [GraphEdge(**e) for e in edges_dict]
...
def merge_graphs(sub_graphs: list[dict[str, Any] | KnowledgeGraph]) -> KnowledgeGraph:
    ...
    for g in sub_graphs:
        if isinstance(g, KnowledgeGraph):
            ...
            all_nodes.extend([GraphNode(**n) for n in g.get("nodes", [])])
            all_edges.extend([GraphEdge(**e) for e in g.get("edges", [])])
...
def apply_entity_mapping(
    ...
    for node in nodes:
        ...
        if canonical_id not in final_nodes:
            final_nodes[canonical_id] = GraphNode(**node_data)
        else:
            # Keep the version with more complete label
